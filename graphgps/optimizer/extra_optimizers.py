...
def modified_state_dict(scheduler):
    original_state_dict = scheduler.state_dict
    def state_dict():
        """Modify the state dict to exclude custom attributes and methods for serialization."""
        state = original_state_dict()
        return {k: v for k, v in state.items() if k not in ['_last_lr', 'get_last_lr']}
    return state_dict
...
def scheduler_reduce_on_plateau(optimizer: Optimizer, reduce_factor: float,
    ...
    # Dynamically adding get_last_lr method if not present
    if not hasattr(scheduler, 'get_last_lr'):
        def get_last_lr(self):
            ...
            if hasattr(self, '_last_lr'):
                ...
                # Fallback if _last_lr has not been set
                return [group['lr'] for group in self.optimizer.param_groups]
        setattr(scheduler, 'get_last_lr', get_last_lr.__get__(scheduler))
        # Initialize _last_lr with the initial learning rates from the optimizer
        setattr(scheduler, '_last_lr', [group['lr'] for group in optimizer.param_groups])

    if not hasattr(scheduler, '_last_lr'):
        setattr(scheduler, '_last_lr', [group['lr'] for group in optimizer.param_groups])


    scheduler.state_dict = modified_state_dict(scheduler)
