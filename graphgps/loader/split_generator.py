...
def setup_standard_split(dataset):
    ...
    if task_level == 'node':
        ...
        print("######## GRAPH ########")
        print(f"Dataset ID in setup_standard_split: {id(dataset)}")

        print("Checking attributes before error")
        print("Train graph index:", hasattr(dataset, 'train_graph_index'))
        print("Validation graph index:", hasattr(dataset, 'val_graph_index'))
        print("Test graph index:", hasattr(dataset, 'test_graph_index'))
...
def set_dataset_splits(dataset, splits):
    ...
    if task_level == 'node':
        ...
        for split_name, split_index in zip(split_names, splits):
            ...
            print(split_index)
            print(split_name)
            print(dataset.data.y.shape[0])


            # Convert the index NumPy array to a PyTorch tensor
            index_tensor = torch.tensor(split_index, dtype=torch.long)

            mask = index2mask(index_tensor , size=dataset.data.y.shape[0])
